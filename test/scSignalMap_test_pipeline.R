...
remotes::install_github('plaisier-lab/scSignalMap/scSignalMap@v1.0')
...
LR_interactions = MapInteractions(seurat1,
                                  'celltype')
write.csv(LR_interactions, "scSignalMap_Testing/LR_interactions.csv")
...
write.csv(de_cond_celltype, "scSignalMap_Testing/de_genes.csv", row.names = FALSE)
...
upreg_receptors = find_upreg_receptors(
    ...
    FC_cutoff = 0.3)
write.csv(upreg_receptors, "scSignalMap_Testing/upreg_receptors.csv", row.names = FALSE)
...
write.csv(upreg_receptors_filtered_and_compared, "scSignalMap_Testing/upreg_receptors_filtered_and_compared.csv", row.names = FALSE)
...
enrichr_results = find_enriched_pathways(
    ...
    adj_p_val_method = "BH",
    adj_p_val_cutoff = 0.05)
write.csv(enrichr_results, "scSignalMap_Testing/enrichr_results.csv", row.names = FALSE)
...
write.csv(master_interaction_list, "scSignalMap_Testing/12.05.25_master_interaction_list.csv", row.names = FALSE)
